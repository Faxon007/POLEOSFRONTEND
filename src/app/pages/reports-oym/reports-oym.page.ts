...
ngOnInit() {
  this.auth.auth.subscribe((session) => {
    ...
    console.log('[OYM-4300] Reportes cargados', robotPermissions);

    if (session.isAuthenticated) {
      this.oymReportsService.getReportsWithFavorites.subscribe(
        (data: IReportByCategory[]) => {
          this.reports = data;
          console.log('[OYM-4300] Reportes cargados', this.reports.length);

          // **Obtenemos query param**
          const openModal = this.route.snapshot.queryParamMap.get('openModal');
          console.log('[OYM-4300] Query param openModal:', openModal);

          if (openModal === 'poleos') {
            console.log('[OYM-4300] Abrir modal Poleos automáticamente');
            this.openPoleosModal();
          }
        }
      )
    }
  ...
  private ChangeValueItem(item: IReport): void {
    let category = this.reports.find((category) => category.id == item.categoryId);
    if (category) {
      if (item.favorite) {
        category.reports = category.reports.filter((report) => report.id != item.id);
      } else {
  ...
  onClickFavorite(item: IReport) {
    if (item.id) {
      if (this.auth.user.favorites?.includes(item.id)) {
        this.auth.user.favorites = this.auth.user.favorites?.filter((reportid) => reportid != item.id);
        item.favorite = false;
        this.ChangeValueItem(item);
        this.reports[0].reports = this.reports[0].reports.filter((report) => report.id != item.id);
        this.event.presentToast('Se elimino de favoritos', 1000, 'danger');
      } else {
        item.favorite = true;
        this.ChangeValueItem(item);
      ...
      if (this.auth.user.id)
        this.userService.updateUniqueParamaterUser(this.auth.user.id, {
  ...
  async openPoleosModal(type: string = 'create', execution?: IExecution) {
    console.log('[OYM] Creando modal de Poleos...');
    ...
    console.log('[OYM] Presentando modal...');
